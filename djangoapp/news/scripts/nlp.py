...
class NLP():
    ...
    def __init__(self) -> None:
        ...
        self.scaler = scaler
        self.proba_cutoff = 0.34093409340934094
        ...
        self.all_quoatation = ['“', '”', '‘', '’', '’', '‘', '’', '‘', u'\u2018', u'\u2019', u'\u201c', u'\u201d', u'\u2032', u'\u2033', '“',  '”',]


    class Word2VecModel():
        def __init__(self) -> None:
            model_settings_path = os.path.join(settings.BASE_DIR, 'news', 'config', 'model_settings.json')
            self.model_settings = self.return_best_model(path=model_settings_path)
        ...
        def return_best_model(self, path: str):
    ...
    class PredictiveModel():
        def __init__(self) -> None:
            model_path = os.path.join(settings.BASE_DIR, 'news', 'predictive_models', 'catboost.pkl')
    ...
    def predict_on_text(self, title:str):
        # print(text)
    ...
    def get_dimensions_to_drop(self):
    ...
    def get_word_vectors(self, w2v_model:Word2VecModel, title:str, aggregation=None):
    ...
    def load_predictive_model(self, path:str):
        classifier = pickle.load(open(path, 'rb'))
        return classifier
        

    def get_stem_of_word(self, text:str):
        return self.stemmer.stem(text)

    def lemmalize_word(self, text:str):
    ...
    def remove_possessive(self, text:str):
        for p in self.all_quoatation:
            text = text.replace(p+'s', '')
            text = text.replace('s'+ p, '')
        return text

    def remove_possessive_nltk(self, text:str):
    ...
    def expand_contractions_nltk(self, text:str):
    ...
    def replace_short_version(self, text:string):
        for p in self.all_quoatation:
    ...
    def replace_numbers_with_words(self, text:list):
        regex = r'\b\d+\b'
    ...
    def remove_non_ascii_characters_from_text(self, text:list):
        return ''.join([i if ord(i) < 128 else ' ' for i in text])

    def remove_non_ascii_characters_from_list(self, text_list:list):
        return [self.remove_non_ascii_characters_from_text(text) for text in text_list]



    def remove_non_eng_words(self, text_list:list, words:list = nltk_corpus_words):
        return [word for word in text_list if word.lower() in words]
        


    def preprocess_title(self, df:pd.DataFrame, verbose = False):
        ...
        df['title'] = df['title'].apply(lambda x: [self.lemmalize_word(word) for word in x])
